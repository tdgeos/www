...
if ($name && $passowrd)
{
 $sql = "SELECT * FROM t_user WHERE (u_UserName = '$name') and (u_Password ='$passowrd')";
 $res = mysql_query($sql);
 $rows = mysql_num_rows($res);
	if($rows)
	{
		$sql = "SELECT `u_UserId`,`u_UserName`,`u_GroupId`,`u_Number` FROM `t_user`";
		$result = mysql_query($sql) or die("查询失败!");
		while ($row=mysql_fetch_array($result)) 
		{
			if($row['u_UserName'] == $name || $row['u_UserId'] == $name)
			{
				$_SESSION['SXWY'] = $row['u_UserId'];
				$_SESSION['land'] = $row['u_UserId'];
				$_SESSION['GroupId'] = $row['u_GroupId'];
				$_SESSION['UserName'] = $row['u_UserName'];
				$_SESSION['Number'] = $row['u_Number']+1;
		   }
		   else if(!$row['u_UserName'] == $name || $row['u_UserId'] == $name)
		   {
				$_SESSION["SXWY"] = $_SESSION['land'] = $_SESSION['GroupId'] = $_SESSION['UserName'] =  $_SESSION['Number'] = 0;  
				?>
